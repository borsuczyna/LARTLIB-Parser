...
export class ReadStream {
        ...
        switch (dataType) {
            case DataType.UInt64:
                if (this.file) {
                    ...
                    result = buffer.readBigUInt64LE(0).toString();
                } else if(this.buffer) result = this.buffer.readBigUInt64LE(this.position).toString();
            ...
            case DataType.Int64:
                if(this.file) {
                    const buffer = Buffer.alloc(readLength);
                    fs.readSync(this.fd!, buffer, 0, readLength, this.position);
                    result = buffer.readBigInt64LE(0).toString();
                } else if(this.buffer) result = this.buffer.readBigInt64LE(this.position).toString();
    ...
    readChars(): string { // read till end of chars (only latin)
        let result = '';
        let char = this.read<string>(DataType.Char);
        
        while(char.charCodeAt(0) >= 32 && char.charCodeAt(0) <= 126) {
            result += char;
            char = this.read(DataType.Char);
        }
        return result;
    }
